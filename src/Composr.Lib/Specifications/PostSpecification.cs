            ...
            RuleSet("Published", () =>
            {
                When(post => post.Status == PostStatus.PUBLISHED, () =>
                {
                    ...
                    RuleFor(post => post.Attributes).Cascade(CascadeMode.StopOnFirstFailure)
                        ...
                        .Must((post, attributes) => !string.IsNullOrWhiteSpace(attributes[PostAttributeKeys.MetaDescription]) && attributes[PostAttributeKeys.MetaDescription].Length >= 115 && attributes[PostAttributeKeys.MetaDescription].Length <= 155).WithMessage("Meta description must be between 115 and 155 characters long");
                });
            });
        }        
    }
